...
% if we haven't run the word cloud data mining, do it now.
if ~exist('sefiData.mat')
    addpath('../datamining');
    % this calls the datamining code. See datamining/SEFIPapersSemanticSurface.m
    nwords = 75;
    ...
    words    = topNWords;
    wordFreq = keywordFrequency(1:nwords);
    save('sefiData.mat', 'words', 'wordFreq', 'correlationMatrix', 'clusterGroups');
else
    load sefiData.mat
end % if(~exist('sefiData.mat'))
